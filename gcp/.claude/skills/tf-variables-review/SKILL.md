...
variable "vpc_zone_count"      { type = number }
variable "vpc_enable_flow_log" { type = bool }
variable "vpc_nat_logging"     { type = bool }
```
...
variable "network" {
  type = object({
    ...
    zone_count      = optional(number, 3)
    enable_flow_log = optional(bool, true)
    nat_logging     = optional(bool, false)
  })
  default = {}
}
```
...
module "vpc" {
  ...
  network = {
    cidr = "172.16.0.0/16"
    # zone_count, enable_flow_log, nat_logging use their field-level defaults
  }
}
```
...
```hcl
variable "network" {
  description = "VPC network configuration. cidr defaults to 10.0.0.0/16. zone_count controls how many zones we span."
  type        = object({ ... })
  default     = {}
}
```
