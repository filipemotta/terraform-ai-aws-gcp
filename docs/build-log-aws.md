...
- While checking the official server's tool list (source, `pkg/tools/**`, v1.3.0): there is
  ...
  pack's `CLAUDE.md` names all four. First finding for the guide, before a single line of
  HCL.
...
Hooks: the guide's `hooks-devops.json` uses a `prompt`-type PreToolUse hook (an LLM judges
the command) and a PostToolUse command that reads `$CLAUDE_TOOL_FILE_PATH`. The current
...
it just is not testable offline. Second finding for the guide: the env var.
...
`README.md`. The monolith's section 9 wants `main.tf` (terraform{} + provider{} only),
`variables.tf`, `outputs.tf`, `datasources.tf`, one kebab-case file per resource group. I
followed the monolith and kept the skill's `README.md` (it costs nothing and the
`tf-outputs-review` skill needs it). Divergence #1 between pack skill and monolith,
recorded in `pack-diff-aws-gcp.md`.
...
`outputs.tf`, `README.md`. Backend key `remote-backend/remote-backend.tfstate` (monolith:
`<stack>/<stack>.tfstate`; the skill says `NN-<name>/terraform.tfstate`, divergence #2).
...
The monolith's section 3 pattern, copied faithfully, does not validate in the workspace a
...
role. Unknown workspaces, `default` included, fail closed. It also gives the monolith's
`assume_role.role_arn` field a job (in the workspace-aware variant it was dead weight).

Validate run 2: `Success! The configuration is valid.` Third finding for the guide.
...
the provider block is identical across stacks, as section 3 demands. `variables.tf`: the
`vpc` object from section 4.2, verbatim in shape, with `enable_dns_support` /
...
`igw-atlas-useast1`, `nat-gateway-atlas-useast1`, `public-subnet-1`… per section 8.
...
- **`Name` only.** Section 7 forbids any tag but `Name` on resources. EKS load balancer
  ...
  and will need one the day an ingress lands. Fourth finding.
...
"network"` (short name, section 11) and `locals` normalising the three values I consume.

`variables.tf`: one `eks` object; managed policy ARNs as lists inside it so
`aws_iam_role_policy_attachment` uses `count` (section 6) instead of one resource per
policy. Policies from the EKS user guide, verified today: cluster `AmazonEKSClusterPolicy`;
node `AmazonEKSWorkerNodePolicy` + `AmazonEC2ContainerRegistryPullOnly` (the guide now
recommends PullOnly over ReadOnly) + `AmazonEKS_CNI_Policy` (the guide recommends moving it
to IRSA; kept on the node role for a floor with no add-ons stack).
...
Labels: the monolith's own example uses `aws_iam_role "eks-node-group"`, a hyphenated label.
...
had put the two `aws_iam_policy_document` data sources in `eks-iam-role.tf`; section 9 says
data sources live in `datasources.tf`. Moved them; validate again `Success!`.

Plan: not executed. Cost (manual, see `measurement.md`): control plane + 2 nodes + EBS.

## Stack 03-data (18:44:54 → ~18:46, 1 validate run)

Two domains in one stack, so two objects: `rds` and `app_bucket` (section 4.2 says one
...
`iam/plan-only-policy.json` is the guide's section 13 policy plus what the S3 backend doc
requires for the lock: `s3:PutObject` + `s3:DeleteObject` allowed only on
`arn:aws:s3:::<bucket>/*.tflock`, explicitly denied on every other key. With the guide's
original policy, `terraform plan` on production dies acquiring the lock. Sixth finding, the
...
Kept the guide's form; live validation is listed as pending in `TESTES.md`.
...
2. **`terraform_remote_state` and workspaces (18:49).** Reading the GCS backend doc for Act
   ...
   at `env:/<workspace>/<key>`. The monolith's section 11 block has no `workspace`
   ...
   Seventh finding, and the most consequential one: the pattern's role selection depends on
   named workspaces, and its cross-stack wiring silently assumes the default one.
