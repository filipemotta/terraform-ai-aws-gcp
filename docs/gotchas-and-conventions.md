...
2. **The thirteen conventions, AWS against GCP**: what the port had to change and what it
   did not, for the stacks and for the pack text.
...
### The pack text: `diff -r aws/.claude gcp/.claude`

Both flavors carry the same thirteen conventions and the fixes of Part 1; every pack file
...
examples. That gives the pack text the same score as the stacks: 8 rules that did not move,
...
| Pack size | an always-on file advertised as "80 lines" shipped at 74, and the count moves with every bootstrap edit | the packs here state no number |
